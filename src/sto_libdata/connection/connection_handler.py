...
from sto_libdata.dataframe_handling.pushable_dataframe import PushableDF, _PushableDataframesWithMetadata, _PushableDataframes, PushableDFWithConfig, PushConfig
from sto_libdata.exceptions.exceptions import NonexistingSchemaError, TableDuplicationError
...
class ConnectionHandler:
    ...
    def __init__(
        ...
        if not self.__schema_exists(self.__metadata):
            raise NonexistingSchemaError(str(self.__metadata.schema), str(self.__con.engine.url.database))
    ...
    def __table_exists(
        ...
        return self.__inspector.has_table(
            table_name = table,
    ...
    def __column_exists(
        ...
        return column in self.__inspector.get_columns(
            table,
            schema=self.__schema_name
        )
    ...
    def download_dataframe(
        ...
        return pd.read_sql_table(
            table_name = table,
            con = self.__con,
            schema = self.__metadata.schema
        )

    def __resolve_table_creation(
        self,
        *pdf_data: _PushData
    ) -> None:
        should_create = (
    ...
    def __handle_if_exists(
        ...
        *pdf_data: _PushData
        ...
        if len(should_fail) > 0:
            raise TableDuplicationError(f"{self.__schema_name}.{should_fail[0].get_name()}")
    ...
    def __resolve_normalization(
        self,
        *pdf_data: _PushData
    ) -> None:
        should_fail_if_not_normalized = (
            df
            for df, config, _ in pdf_data
            if config.fail_if_not_normalized
        )
    ...
    def push_tables(
        ...
        *pushable_dataframes: PushableDF | PushableDFWithConfig,
        ...
        pdfs = _PushableDataframes(
            *(pdf for pdf, _, _ in pushable_dataframes_data)
        )

        pdfs.insert(self.__schema_name, self.__con)
        self.commit_changes()

    def __drop_tables_no_check(
        self,
        *table_name : str
    ) -> None:
        tables = [Table(name, self.__metadata, autoload_with=self.__con) for name in table_name]
